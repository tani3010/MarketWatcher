...
if __name__ == '__main__':
    mgr = CommitmentsOfTradersShortFormatTableManager()
    sql = 'select * from TBL_COMMITMENTSOFTRADERS_SHORTFORMAT'
    # sql = "select * from TBL_COMMITMENTSOFTRADERS_SHORTFORMAT where Report_Date_as_YYYY_MM_DD = '2021-04-20' and FutOnly_or_Combined = 'FutOnly'"
    dat = mgr.select(sql, True)
    df = pd.DataFrame(dat[1:], columns=dat[0])
    df.to_csv('testDF_short.csv', index=False)
