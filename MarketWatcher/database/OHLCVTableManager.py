...
class OHLCVTableManager(BaseDataBaseTableManager):
    ...
    def resample(self, df, freq, drop_na=False):
        ...
        df = df.resample(freq).agg(dict_ohlc)
        if drop_na:
            df = df.dropna()
        else:
            df = df.ffill()
    ...
    def fetch_OHLCV(self, exchange_name, symbol, freq=None, from_date='1950-01-01', to_date='2100-12-31', drop_na=False):
        sql = '''
        select
          datetime,
          Open,
        ...
        df = self.set_datetime_as_index(df)
        if freq is not None:
            df = self.resample(df, freq, drop_na)
        return df
