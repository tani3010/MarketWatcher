...
class BaseTrader(object):
    ...
    def fetch_fee(self, result, symbol, price, amount, takerOrMaker='taker', params={}):
        ...
        try:
            ...
            tmp_result = self.exchange.calculate_fee(symbol, type_, side, amount, price, takerOrMaker, params)
            self.copy_dict(tmp_result, result)
            ret = True
            logger.info('completed.')
        except Exception as e:
    ...
    def calculate_tradable_amount(self, result, symbol, price, side, type, takerOrMaker='taker', safe_margin=0.05):
        ret = False
        result_balance = None
        result_fee = None
