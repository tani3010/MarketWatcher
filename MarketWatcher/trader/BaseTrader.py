...
class BaseTrader(object):
    ...
    def fetch_fee(self, result, symbol, price, amount, takerOrMaker='taker', params={}):
        ret = False
        try:
            type = None  # this is not used in public API
            side = None  # this is not used in public API
            tmp_result = self.exchange.calculate_fee(symbol, type, side, amount, price, takerOrMaker, params)
            self.copy_dict(tmp_result, result)
            ret = True
            logger.info('completed.')
        except Exception as e:
    ...
    def calculate_tradeble_amount(self, result, symbol, price, side, type, takerOrMaker='taker', safe_margin=0.05):
        result_balance = None
        result_fee = None
        if not self.fetch_balance(result_balance):
            pass

        if not self.fetch_balance(result_fee):
            pass
