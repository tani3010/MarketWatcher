            ...
            for iURL in range(len(self.URL_BASE)):
                for symbol in self.TARGET_SYMBOLS[iURL]:
                    ...
                    while has_next:
                        ...
                        # df = df_tmp if df is None else df.append(df_tmp, ignore_index=True)
                        df = df_tmp if df is None else pd.concat([df, df_tmp], ignore_index=True)
                    ...
                    df['updatetime'] = time()
                    df_daily_bybit = df if df_daily_bybit is None else pd.concat([df_daily_bybit, df], sort=True)
