...
class Phemex(BaseSelenium):
    ...
    def update_phemex_fundingRate(self):
        ...
        # tmp = self.driver.find_elements_by_xpath("//li[@class='pr T2 cp ph10 svelte-1oj3pur']")
        # tmp = self.driver.find_elements_by_xpath("//li[@class='pr T2 cp ph10 svelte-1ttcrzp']")
        # tmp = self.driver.find_elements_by_xpath("//li[@class='pr T2 cp ph10 svelte-1danmgl']")
        tmp = self.driver.find_elements_by_xpath("//li[@class='pr T2 cp ph10 svelte-128wrzt']")
        ...
        for i in range(2, product_count+2):
            ...
            xpath = r'/html/body/div[3]/div/div/div/div[1]/div/div[2]/div/i'
            xpath = r'/html/body/div[2]/div/div/div/div[1]/div/div[2]/div/i'
            btn = self.driver.find_element_by_xpath(xpath)
            btn.click()
            self.wait_expected_condition()

            xpath = r'/html/body/div[3]/div/div/div/div[1]/div/div[2]/ul/li[{}]'.format(i)
            xpath = r'/html/body/div[2]/div/div/div/div[1]/div/div[2]/ul/li[{}]'.format(i)
            btn = self.driver.find_element_by_xpath(xpath)
            ...
            try:
                ...
                while True:
                    # fr_web_list = self.driver.find_elements_by_css_selector(".td.T2.svelte-q8200j")
                    ...
                    for j in range(0, len(fr_web_list), 4):
                        ...
                        if timestamp != '1969-12-31 19:00:00':
                            ...
                            # df_daily_phemex = pd.DataFrame(tmp_dict, index=[0]) if df_daily_phemex is None else df_daily_phemex.append(tmp_dict, ignore_index=True)
                            df_daily_phemex = pd.DataFrame(tmp_dict, index=[0]) if df_daily_phemex is None else pd.concat([df_daily_phemex, pd.DataFrame(tmp_dict, index=[0])], ignore_index=True)
