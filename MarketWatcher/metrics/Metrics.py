...
class Metrics(object):
    ...
    @staticmethod
    def resample(df_ohlcv, rule):
        return df_ohlcv.resample(rule, label='right').agg(OHLCV_AGG).dropna()
    ...
    def dummy(self, df, rule, column, **kwargs):
        return resample_apply(rule, self._dummy, df[column], **kwargs)
    ...
    @staticmethod
    def MACD(df_ohlcv, rule, fastperiod=12, slowperiod=26, signalperiod=9):
        return resample_apply(
